  ...
  WHERE
    cats.breed = (
      ...
      FROM
        cats
      WHERE
        cats.color = 'Lavender' AND cats.name = 'Johnson'
    )
  ORDER BY
    cats.name ASC;
  SQL
...
#   cats AS johnson_cat
# JOIN
#   cats AS compared_cats ON johnson_cat.color = 'Lavender' AND johnson_cat.name = 'Johnson'
# WHERE
#   compared_cats.breed = johnson_cat.breed
# ORDER BY
#   compared_cats.name ASC;
